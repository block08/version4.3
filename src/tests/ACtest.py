...
class Game:
    ...
    def run(self):
        ...
        PARTICIPANT_ID_FOLDER = "subA+C"  # 本次训练的特定文件夹
        TOTAL_IMAGES = 3
        ...
        user_id3 = re.search(r'\d+$', user3_mark).group() if re.search(r'\d+$', user3_mark) else user3_mark
        game_drawing = GameDrawing()

        timestamps, time_records = [None] * TOTAL_IMAGES, [None] * (TOTAL_IMAGES + 1)
        with open('config.txt', 'w') as f:
            f.truncate(0); f.write('3')
        ...
        # 【关键】自动显示第一张图片
        time_records[0], timestamps[0] = game_drawing.random_painting(numbers[0], self, 11)
        ...
        while running:
            ...
            Button3(settings, self.screen, f"航天员：{user_id1} & {user_id3}", 10, 20).draw_button()
            ...
            if paused:
                ...
                current_img_num = stats.game_score - 23
            ...
            # ------------------- 任务结束处理 -------------------
            if stats.game_score == 23 + TOTAL_IMAGES + 1:
                self.level.clear()
    ...
    def display_task_instructions_formatted(self, subject='AC'):
        """以标准风格动态绘制AC合作任务的指导语"""
        ...
        title_text = f"{user1}和{user3}合作绘图练习指导语"
        ...
        role_text = f"请 {user1} 和 {user3} 合作完成任务"

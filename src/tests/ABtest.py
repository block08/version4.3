...
class Game:
    ...
    def run(self):
        ...
        user_id1 = extract_number(user1_mark)
        user_id2 = extract_number(user2_mark)
        ...
        while running:
            ...
            # --- 1. 事件处理 ---
            mouse_pos = pygame.mouse.get_pos()
            user_button = Button3(settings, self.screen, f"航天员：{user_id1}&{user_id2}", 10, 20)
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_ESCAPE:
                        ...
                        if stats.game_score == 23:
                            # 第一张图片，直接开始
                            action_pending = True
                        elif 24 <= stats.game_score < 26 and self.level.is_endpoint_reached():
                            # 后续图片需要到达终点才能继续
                            action_pending = True
                        elif stats.game_score == 26 and self.level.is_endpoint_reached():
                            # 最后一张图片完成
                            action_pending = True
            ...
            if action_pending:
                if not first_image_shown:
                    t1, timestamp1 = game_drawing.random_painting(numbers[0], self, 11)
                    ...
                    pygame.image.save(self.screen, f"{output_image_folder}/post_screenshot{current_image_index - 1}.png")
                    if stats.game_score == 24:
                        t2, timestamp2 = game_drawing.random_painting(numbers[1], self, 24)
                        pygame.image.save(self.screen, f"{output_image_folder}/pre_screenshot1.png")
                    elif stats.game_score == 25:
                        t3, timestamp3 = game_drawing.random_painting(numbers[2], self, 25)

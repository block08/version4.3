...
class Game:
    ...
    def render_text_with_green_keys(self, text, font, surface, center_pos):
        """渲染带有高亮按键提示的文本"""
        GREEN_COLOR, BLACK_COLOR = (0, 0, 255), (0, 0, 0)
    ...
    def run(self):
        print("[ABtest] 开始运行合作绘图")
        # ------------------- 初始化设置 -------------------
        print("[ABtest] 正在初始化设置...")
        settings = Settings()
        stats = GameStats(settings)
        print("[ABtest] 设置初始化完成")
        ...
        while running:
            ...
            if paused:
                ...
                if 1 <= current_img_num <= 2:
                    step_button.text = f"{current_img_num} / {TOTAL_IMAGES}"
                    hint_text = "按P键暂停"
                    center_button.text = "按空格键进入下一试次" if self.level.is_endpoint_reached() else ""
                    self.render_text_with_green_keys(hint_text, key_hint_font, self.screen,
                                                     (settings.screen_width // 2, 60))
                    step_button.draw_button()
                    if center_button.text: center_button.draw_button()
                elif current_img_num == 3:  # 任务完成
                    step_button.text = f"{current_img_num} / {TOTAL_IMAGES}"
                    hint_text = "按P键暂停"
                    center_button.text = "按空格键结束绘图" if self.level.is_endpoint_reached() else ""
                    self.render_text_with_green_keys(hint_text, key_hint_font, self.screen,
                                                     (settings.screen_width // 2, 60))
                    step_button.draw_button()
                    if center_button.text: center_button.draw_button()
    ...
    def display_task_instructions_formatted(self, subject='AB'):
        ...
        def draw_keyboard_layout(screen, center_x, top_y, key_font, is_highlighted, subject):
            ...
            if subject in ['A', 'B', 'C']:  # 单人模式
                ...
                # 绘制辅助
                user2_surf = body_font.render("辅助", True, TEXT_COLOR)
        ...
        # 计算最长标题的宽度，用于对齐冒号
        user_text = {'A': user1, 'B': user2, 'C': user3, 'AB': f"{user1}和辅助", 'AC': f"{user1}和{user3}"}.get(
            subject, "")
        ...
        if subject == 'A':
            ...
            content_text = f"{user1}控制A键向左，D键向右； 辅助控制↑键向上，↓键向下"
    ...
    def display_end_screen(self, is_training=False):
        """显示标准化的结束画面"""
        BG_COLOR, TEXT_COLOR, ACCENT_COLOR = (230, 230, 230), (0, 0, 0), (0, 0, 255)
...
def draw_data(self, screen, data):
    ...
    line2 = f"任务总用时：{sum(times):6.2f}秒"
